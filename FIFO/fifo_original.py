...
writer_final = pd.ExcelWriter(nombrearchivo_final, engine='openpyxl')
resultado_FIFO_CD.to_excel(writer_final,'FIFO CDA', index=False)
descuento_DDA48_df.to_excel(writer_final,'DESCUENTO DD48', index=False)
emp_final_df.to_excel(writer_final, 'EMPUJES', index=False)
resultado_suc_sat.to_excel(writer_final,'FIFO SUC SAT', index=False)
cant_total_palet.to_excel(writer_final,'CANT PALLET', index=False)
writer_final.book.save(nombrearchivo_final)
